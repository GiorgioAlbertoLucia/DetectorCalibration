...
def prepare_dataset(dataset, particle:str, cfg):
    ...
    dataset['fPt'] = np.abs(dataset[cfg['pt']])
...
def main():
    parser = argparse.ArgumentParser(description='TOF calibration')
    parser.add_argument(
        '--config', '-c', required=True,
        help='Path to YAML config file (e.g. configs/LHC23_pass4_TOF.yaml)',
    )
    ...
    calibrator = TOFCalibrator(cfg, dataset)

    for particle in cfg['tof']['particles']:
